...
#define CHAR_WIFI_DELIMETER ','
#define CHAR_WIFI_DELIMETER_END ':'
...
void printReadSectionBuf(struct struct_buf *buffer);
void stripWiFiPrefix();
    ...
    switch (stage) {
        ...
        case 19:
        {
            ...
            if (wifiResponseCheck("+IPD,0,12:!ping%430*", "null") == true || wifiResponseCheck("+IPD,0,11:!ping%430*", "null") == true) {
                ...
                UART2SendString( "banana" );
                int i = bufRecvWiFi.posRead;
                char test = bufRecvWiFi.buf[ bufRecvWiFi.posRead ];
                UART2SendString( test );
                //printReadSectionBuf( &bufRecvWiFi );
                
                // stripWiFiPrefix();
                // strcat( bufRecvWiFi.buf);
            ...
            if (wifiResponseCheck("WIFI DISCONNECT", "null") == true) {
                ...
                LED1SET = 0;
                UART2SendString("Lost Connection");
                stage = 0;
            ...
            while (bufRecvWiFi.buf[bufRecvWiFi.posRead ] != CHAR_NULL) {
                ...
                if (inCommand == true) {
                    ...
                    if (bufRecvWiFi.buf[bufRecvWiFi.posRead ] == '*') {
                        ...
                        UART2SendString( bufRecvWiFiCommand ); //<>
                        bufRecvWiFiCommandPos = 0;
                    }
                }
...
void printReadSectionBuf(struct struct_buf *buffer)
{
    ...
    UART2SendString("gothere1");
    int inx = 0;
    int jnx = buffer->posRead;
    
    UART2SendString("gothere2");
    while ( buffer->buf[jnx] != CHAR_NULL )
    {
        UART2SendString("gothere3");
        UART2SendString( &inx );
        message[inx] = buffer->buf[jnx];
        inx++;
        jnx++;
        
    }
    
    UART2SendString( messageptr );
}

void stripWiFiPrefix() 
{

    char c;       // Place to store the prefix read off of the command
    
    // Example Command: +IPD,0,14:!Set;En;4000*
    // Example Prefix:  +IPD,0,14:
    // Stripped command:          !Set;En;4000*

    char connectionID[] = "+IPD";   // Connection tag, used to see if the prefix is valid.
    char session[] = "0";           // Session number, used to see if the command is valid.
    int d2 = 0;                     // Number of times the second delimeter was found.
    ...
    // While the buffer has not reached end-of-character,
    // loop through each character in the buffer.
    while ( (bufRecvWiFi.buf[ bufRecvWiFi.posRead ] != CHAR_NULL) && (endLoop == false) )
    {

        if (bufRecvWiFi.posRead >= BUF_LEN) 
        {
            bufRecvWiFi.posRead = 0;
        }

        // The character in the buffer should be added to the stored prefix.
        c = bufRecvWiFi.buf[ bufRecvWiFi.posRead ];
        bufRecvWiFi.posRead++;

        // If the selected character in the prefix is the
        // end-of-prefix delimeter, end the loop after the following
        // set of vaildation checks.
        if (c == CHAR_WIFI_DELIMETER_END)
        {
            endLoop = true;
        }


        // If a prefix token delimeter is found, note it and go to the next character.
        if ( CHAR_WIFI_DELIMETER == c )
        {
            d2++;
            inx++;

            // If the session is not listed
            if ( (d2 == 2) && (inx - sizeof(connectionID)) != 2 )
            {
        ...
        // PREFIX VALIDATION SECTION

        // Connection ID: First token in the prefix
        if (d2 == 0)
        {

            // If the prefix is shorter than the connection ID, invalidate prefix
            if ( inx > sizeof(connectionID) )
            {
                containsPrefix = false;
            }

            // If the first term in the prefix doesn't match the connection ID, invalidate prefix
            if ( connectionID[inx] != c )
            {
                containsPrefix = false;
            }
        ...
        // Session ID: Second token in the prefix
        else if (d2 == 1) {
            ...
            // If the connection isn't on session 0, invalidate prefix
            if (c != session[0])
            {
                containsPrefix = false;
            }
        ...
        // If there are too many commas, invalidate prefix
        else if (d2 >= 3)
        {
            containsPrefix = false;
        }
        ...
        // If the prefix is invalid, end the loop.
        if (containsPrefix == false)
        {
            bufRecvWiFi.posRead = bufRecvWiFi.posWrite;
            endLoop = true;
        }
